...
- В каталог 'data/tmp/' будет сохранен файл в котором будет список файлов для восстановления.
ВАЖНО! Этот каталог и файлы в нем должны быть доступны Директору по чтению.
...
Поддерживаются СУБД MySQL и PostgreSQL.
   ...
   Запустите необходимые скрипты из каталога update/
...
CREATE DATABASE webacula 
   ...
   DEFAULT COLLATE utf8_general_ci;
    ...
    Время создания временных таблиц Webacula зависит от:
    - настроек вашей РСУБД для оптимизации INSERT'ов
    - количества уникальных путей
    - количества уникальных имен файлов
...
Примечание. Этот процесс может занять продолжительное время.
    ...
    И пропорционально увеличить "ttl_restore_session" в application/config.ini
